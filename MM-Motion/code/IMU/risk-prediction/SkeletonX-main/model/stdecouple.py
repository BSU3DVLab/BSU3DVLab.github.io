...
class Model(nn.Module):
    def __init__(self, gcn_model, loss_crit, aggr_mode, n_channel, n_frame, n_joint, n_person, **kwargs):
        ...
        self.class_weights = getattr(self, 'class_weights', None)
    ...
    def loss(self, output, label, losses, label_dasp=None, label_sadp=None, label_dasp_mix=None, label_sadp_mix=None, tb_writer=None, weight_dict=None, global_step=None):
        ...
        def weighted_loss(logits, targets, weights):
            """应用类别权重的交叉熵损失"""
            unweighted_loss = self.loss_crit(logits, targets)
            if weights is not None:
                # 根据标签获取对应权重
                sample_weights = weights[targets]
                # 对每个样本应用权重
                weighted = unweighted_loss * sample_weights
                return weighted.mean()
            return unweighted_loss.mean()

        acc = {}
        # calculate loss for training samples
        losses['loss_origin'] = weighted_loss(output['x'], label, self.class_weights)
