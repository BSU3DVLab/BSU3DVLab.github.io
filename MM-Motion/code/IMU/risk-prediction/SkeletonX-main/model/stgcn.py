...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_frame=64, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        # data_bn 维度基于实际输入维度 in_channels * num_point，而非 num_person
        # 这样当 M=1 时，展平维度 M * V * C 也能匹配
    ...
    def forward_to_hidden_feat(self, x):
        ...
        # 支持两种输入格式:
        # 1. [N, C, T, V, M] - 标准格式
        # 2. [N, T, V, C] - 来自SubjectPairDataset的格式，需要转换
        
        if x.dim() == 4:
            # [N, T, V, C] -> [N, C, T, V, M]
            N, T, V, C = x.shape
            x = x.permute(0, 3, 1, 2).unsqueeze(-1)  # [N, C, T, V, M]
