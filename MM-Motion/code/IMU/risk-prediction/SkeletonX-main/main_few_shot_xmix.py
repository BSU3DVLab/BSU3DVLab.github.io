...
# Only set resource limits on Unix/Linux systems
...
def to_device(tensor, device):
    """Move tensor to specified device, handling CPU mode"""
...
class Processor:
    ...
    def load_model(self):
        ...
        # print(gcn_model)
        self.loss = build_loss(self.arg).to(output_device)

        self.rank_criterion = nn.MarginRankingLoss(margin=0.05).to(output_device)
        self.softmax_layer = nn.Softmax(dim=1).to(output_device)
    ...
    def load_optimizer(self):
        # 计算类别权重用于处理数据不平衡
        if hasattr(self, 'data_loader') and 'aux' in self.data_loader:
            aux_dataset = self.data_loader['aux'].dataset
            if hasattr(aux_dataset, 'label'):
                class_weights = get_class_weights(aux_dataset.label)
                self.class_weights = class_weights.to(self.output_device)
                self.print_log(f'Class weights: {class_weights}')
                # 将类别权重传递给模型
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            with torch.no_grad():
                data = to_device(data.float(), self.output_device)
                label = to_device(label.long(), self.output_device)
            ...
            if self.arg.x_SADP:
                ...
                with torch.no_grad():
                    pair_data_SADP = to_device(torch.from_numpy(pair_data).float(), self.output_device)
                    pair_label_SADP = to_device(torch.from_numpy(pair_label).long(), self.output_device)
            ...
            if self.arg.x_DASP:
                ...
                with torch.no_grad():
                    pair_data_DASP = to_device(torch.from_numpy(pair_data).float(), self.output_device)
                    pair_label_DASP = to_device(torch.from_numpy(pair_label).long(), self.output_device)
                timer['pairloader'] += self.split_time()
    ...
    def eval(self, epoch, save_score=False, wrong_file=None, result_file=None, mask_subject=False):
        ...
        for (data, label, index) in self.data_loader['anchor']:
            with torch.no_grad():
                data = to_device(data.float(), self.output_device)
                anchor_label = to_device(label.long(), self.output_device)
